...
const service = {
  ...
  filtrar: async (objData) => {
    let filteredData = data.filter((filteredItem) => {
      let validacaoDescricao =
        !objData.descricao || (objData.descricao && filteredItem.descricao.indexOf(objData.descricao) !== -1);
      let validacaoIdentidade =
        !objData.identidade || (objData.identidade > 0 && filteredItem.identidade === objData.identidade);
  ...
  buscar: async (id) => {
    return await Promise.resolve({
      status: 200,
      data: { content: data.find((item) => item.id === id), success: true, error_message: null },
    });
  },
  cadastrar: async (item) => {
    ...
    return await Promise.resolve({
      status: 200,
      data: { content: data.find((i) => i.id === item.id), success: true, error_message: null },
    });
  },
  deletar: async (id) => {
